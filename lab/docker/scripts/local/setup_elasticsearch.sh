...
until curl -u "${ADMIN_USER}:${ADMIN_PASS}" -s http://localhost:9200/_cluster/health | grep -qE '"status":"(yellow|green)"'; do
  sleep 5
...
curl -s -u "${ADMIN_USER}:${ADMIN_PASS}" -X POST "http://localhost:9200/_security/role/${READONLY_ROLE}" -H "Content-Type: application/json" -d '{
  "cluster": ["monitor"],
  "indices": [
    {
      "names": ["*"],
...
curl -s -u "${ADMIN_USER}:${ADMIN_PASS}" -X POST "http://localhost:9200/_security/user/${READONLY_USER}" -H "Content-Type: application/json" -d '{
