...
check_elasticsearch_index() {
  ...
  if curl -s -u "$ES_USER:$ES_PASS" -o /dev/null -w "%{http_code}" "http://${ELASTIC}:9200/$index" | grep -q "200"; then
...
create_elasticsearch_index() {
  ...
  curl -s -X PUT -u "$ES_USER:$ES_PASS" "http://${ELASTIC}:9200/$index" \
...
setup_kibana_index() {
  ...
  curl -s -X POST -u "$ES_USER:$ES_PASS" "http://${KIBANA}:5601/api/saved_objects/index-pattern" \
    -H "kbn-xsrf: true" \
    -H "Content-Type: application/json" \
    -d '{
      "attributes": {
...
# === Execução Principal ===
wait_for_service "Elasticsearch" "http://${ELASTIC}:9200"
